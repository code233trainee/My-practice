...
template <typename T> void RedBlack<T>::solveDoubleBlack( BinNodePosi<T> r ) {
   ...
   if ( IsBlack( s ) ) { //兄弟s为黑
      ...
      // 以下，通过旋转重平衡，并将新子树的左、右孩子染黑
         ...
         if ( HasLChild( *b ) ) { b->lc->color = RB_BLACK; b->lc->updateHeight(); } //左子
         if ( HasRChild( *b ) ) { b->rc->color = RB_BLACK; b->rc->updateHeight(); } //右子
         b->color = oldColor; b->updateHeight(); //新子树根节点继承原根节点的颜色
